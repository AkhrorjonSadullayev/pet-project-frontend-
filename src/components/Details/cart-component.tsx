  ...
  <tbody>
    {cartData.todo.map((item: any, index: number) => (
      <tr key={item._id} className="cart-tr-body">
        ...
        <td>
          <button
            onClick={() => updateCartQuantity(item.product, -1, item.price)} // Kamaytirish
          ...
          <button
            onClick={() => updateCartQuantity(item.product, 1, item.price)} // Oshirish
            style={{
              padding: "5px 10px",
              backgroundColor: "#7F4D4F",
              fontWeight: "bold",
        ...
        <td>
          <button
            className="open"
            onClick={() => handleOpen(item._id)} // Modalni faqat tegishli item uchun ochish
          >
            Delete
          </button>
          <Modal
            open={open === item._id} // Modal faqat tegishli item uchun ochiladi
            onClose={handleClose}
            aria-labelledby="modal-modal-title"
            aria-describedby="modal-modal-description"
          >
            <Box sx={{
              position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 500, bgcolor: 'background.paper', border: '2px solid brown', boxShadow: 24, p: 4,
              ...
              <div>
                ...
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "20px" }}>
                  <button
                   onClick={() => { deleteItemFromCart(item.product._id, item._id); handleClose(); }}
                    style={{
                      width: "80px", height: "40px", backgroundColor: "#c0392b ", color: "white", border: "none", borderRadius: "5px", cursor: "pointer"
                    }}
                  >
                    YES
                  </button>
                  <button
                    ...
                    className="noBtn"
                    style={{
                      width: "80px", height: "40px", backgroundColor: "#1e8449 ", color: "white", border: "none", borderRadius: "5px", cursor: "pointer"
                    }}
                  >
                    NO
                  </button>
    ...
    <div style={{ maxWidth: "1400px", width: "100%", display:'flex',justifyContent:"flex-end",marginTop:'20px'}}>
      <div className="total">
