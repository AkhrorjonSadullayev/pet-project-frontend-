...
import {
  ...
  TabContainer,
} from "./styles/parrotDetail.style";
...
const ParrotFoodDetail: React.FC = () => {
  ...
  const relatedParrotFood = products.filter((item)=> ["Parrot Food","Parrot"].includes(item.type))
  const likeParrotFood = products.filter((item) => ["Fish Food","Fish"].includes(item.type))
  return (
    <MainContainer>
      <FilterDataCon>
        {/* Header */}

        <div className="parrot-food-detail-back">
          <h1>{product.title}</h1>
      ...
      <AlsoLikeContainer>
        <div className="grid">
        {relatedParrotFood.map((item,index) => (
            <div key={index} >
...
export default ParrotFoodDetail;
