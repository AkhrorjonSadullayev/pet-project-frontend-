...
import { FilterDataCon, MainContainer, TabContainer } from "./styles/petToysDetail.style";
...
import { Accordion, AccordionDetails, AccordionSummary, Alert, Button, Snackbar, SnackbarCloseReason, Tab, TextField, Typography } from "@mui/material";
import { AlsoLikeContainer, RelaitedProContainer } from "./styles/petToysDetail.style";
...
const PetToysDetail = () => {
  ...
  const relatedPetToys = products.filter((item) => item.type === "Pet Toys");
  const likedPetToys = products.filter((item) => item.type === "Accessories");
  ...
  return (
    <MainContainer>
      <FilterDataCon>
        ...
        <div className="pet-toys-detail-back">
          <h1>{product.title}</h1>
      ...
      <AlsoLikeContainer>
        <div className="grid">
        {relatedPetToys.map((item,index) => (
            <div key={index} >
              <div key={item._id} className="product-img-con">
                <Link
                  to={`/pet-food/pet-toys/${item._id}`}
      ...
      <RelaitedProContainer>
        <div className="grid">
        {likedPetToys.map((item,index) => (
            <div key={index} >
              <div key={item._id} className="product-img-con">
                <Link
                  to={`/pet-food/accessories/${item._id}`}
                  style={{ textDecoration: "none" }}
                >
                  <img className="img-class" src={item.main} alt="product-img" />
                  <div className="price">
