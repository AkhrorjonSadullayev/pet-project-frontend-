...
import {
  ...
  TabContainer,
} from "./styles/accessoriesDetail.style";
...
const AccessoriesDetail: React.FC = () => {
  ...
  const relatedAccess = products.filter((item)=> item.type === "Accessories")
  const likeAccess = products.filter((item)=> item.type === "Pet Toys")
  return (
    <MainContainer>
      <FilterDataCon>
        {/* Header */}

        <div className="accessories-detail-back">
          <h1>{product.title}</h1>
