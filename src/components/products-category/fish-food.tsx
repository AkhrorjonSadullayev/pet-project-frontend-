...
import "./styles/fish-food.style";
import { MainContainer } from "./styles/fish-food.style";
...
const FishFoodComponent = () => {
  ...
  const filteredOriginal = products.filter((item) => {
   ...
   const matchesType = item.type === "Fish"
  ...
  const filteredOriginalF = products.filter((item) => {
    const isWithinPriceRange = item.price >= value[0] && item.price <= value[1];
   const matchesType = item.type === "Fish Food"
    const matchesSelectedColors =
      selectedColors.length === 0 || selectedColors.includes(item.color);
    const matchesSelectedLocation =
      selectedLocation.length === 0 || selectedLocation.includes(item.location);
  ...
  return (
    <MainContainer>
            ...
            <AccordionDetails>
              <div>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                    checked={selectedType.includes("Fish Food")}
                    onChange={() => handleTypeChange("Fish Food")}
                      sx={{
                        color: "black",
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                    checked={selectedType.includes("Fish")}
                    onChange={() => handleTypeChange("Fish")}
                      sx={{
                        color: "black",
                        "&.Mui-checked": { color: "black" },
                      }}
      ...
      <div key={index} className="product-img-con">
        <Link
          to={`/${
            value.type === "Fish" ? "others" : "fish-food"
          }/${value._id}`}
...
export default FishFoodComponent;
