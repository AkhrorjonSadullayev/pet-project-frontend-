...
const DogDetail: React.FC = () => {
  ...
  const relatedDog = products.filter((item) => item.type === "Cat");
  const likeCatFood = products.filter((item) => item.type === "Dog Food");
  const handleMouseMove = (e: React.MouseEvent<HTMLImageElement>) => {
    const { left, top } = e.currentTarget.getBoundingClientRect();
    setCursorPos({
      x: ((e.clientX - left) / e.currentTarget.width) * 100, // X koordinatlarini foizda hisoblash
      y: ((e.clientY - top) / e.currentTarget.height) * 100, // Y koordinatlarini foizda hisoblash
    });
  };
  return (
    <MainContainer>
        ...
        <div className="product-detail-container">
          ...
          <div className="data-img-info">
            <div className="data-info-con">
              ...
              <div className="zoom-container" style={{ position: "relative" }}>
                <div
                  className="image-wrapper"
                  style={{
                    width: "500px", // Sizning tasvir kengligi
                    height: "500px", // Sizning tasvir bo'yi
                    overflow: "hidden",
                    position: "relative",
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      objectFit: "cover",
                      borderRadius: "10px",
                      transformOrigin: `${cursorPos.x}% ${cursorPos.y}%`, // Kursor pozitsiyasiga qarab zoom qiladi
                      transform: isZooming ? "scale(1)" : "scale(1)", // Zoom qilish effekti
                    }}
                    onMouseEnter={() => setIsZooming(true)} // Hoverda zoom boshlanadi
                    onMouseLeave={() => setIsZooming(false)} // Hoverdan chiqqanda zoom tugaydi
                    onMouseMove={handleMouseMove} // Kursorni harakatlantirish
                  />
                </div>
                ...
                <div
                  ...
                  style={{
                    display: isZooming ? "block" : "none", // Zoom holatida tasvirni ko'rsatish
                    position: "absolute",
                    top: "0",
                    left: "100%",
                    width: "500px", // Sizning tasvir kengligi
      ...
      <RelaitedProContainer>
        <div className="grid">
          {likeCatFood.map((item, index) => (
            <div key={index}>
              <div className="product-img-con">
                <Link
                  to={`/pet-food/dog-food/${item._id}`}
                  style={{ textDecoration: "none" }}
                >
                  <img
                    className="img-class"
